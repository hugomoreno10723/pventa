...
if (isset($_POST) && count($_POST)>0)
{
	...
	{
		$query=$db->query("update producto set ".$_POST["campo"]."='".$_POST["valor"]."' where id='".intval($_POST["id"])."' limit 1");
...
if (isset($_GET) && count($_GET)>0)
{
	...
	{
		$query=$db->query("select * from producto order by id asc");
		$datos=array();
		while ($usuarios=$query->fetch_array())
		{
			$datos[]=array(	"nombre"=>$usuarios["nombre"],
							"caracteristicas"=>$usuarios["caracteristicas"],
